...
$key1 = $_POST['key1'] ?? ($_GET['key1'] ?? '');  // 查詢的資料
$key2 = $_POST['key2'] ?? ($_GET['key2'] ?? '');  // 查詢的資料
...
$sqlstr .= "WHERE username LIKE '%$key1%' ";
$sqlstr .= " AND address LIKE '%$key2%' ";
...
$navigator = "";
for($i=1; $i<=$page-1; $i++) {
   $navigator .= "<a href=\"?key1=$key1&key2=$key2&page=" . $i . "\">" . $i . "</a>&nbsp;";
}
$navigator .= "[" . $i . "]&nbsp;";
...
$lnk_pageprev  = "?key1=$key1&key2=$key2&page=" . (($page==1)?(1):($page-1));
$lnk_pagenext  = "?key1=$key1&key2=$key2&page=" . (($page==$total_page)?($total_page):($page+1));
$lnk_pagefirst = "?key1=$key1&key2=$key2&page=1";
$lnk_pagelast  = "?key1=$key1&key2=$key2&page=" . $total_page;
