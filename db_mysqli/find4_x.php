...
$type = $_POST['type'] ?? ($_GET['type'] ?? '');  // 查詢類型 
$key = $_POST['key'] ?? ($_GET['key'] ?? '');  // 查詢的資料
...
switch($type) {
   ...
   case 'ADDR':
      $sql_where = "WHERE address LIKE '%$key%' ";
      break;
...
$navigator = "";
for($i=1; $i<=$page-1; $i++) {
   $navigator .= "<a href=\"?type=$type&key=$key&page=" . $i . "\">" . $i . "</a>&nbsp;";
}
$navigator .= "[" . $i . "]&nbsp;";
...
$lnk_pagefirst = "?type=$type&key=$key&page=1";
